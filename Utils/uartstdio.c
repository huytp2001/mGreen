...
UARTStdioConfig(uint32_t ui32PortNum, uint32_t ui32Baud, uint32_t ui32SrcClock)
{
    ...
    //
    MAP_UARTFIFOLevelSet(g_ui32Base, UART_FIFO_TX1_8, UART_FIFO_RX7_8);
...
int
UARTwriteData(uint8_t* pu8_data, uint32_t ui32Len)
{
#ifdef UART_BUFFERED
    unsigned int uIdx;
    ...
    for(uIdx = 0; uIdx < ui32Len; uIdx++)
    {
        ...
        if(!TX_BUFFER_FULL)
        {
            g_pcUARTTxBuffer[g_ui32UARTTxWriteIndex] = *(pu8_data + uIdx);
            ADVANCE_TX_BUFFER_INDEX(g_ui32UARTTxWriteIndex);
        }
        else
        {
    ...
    for(uIdx = 0; uIdx < ui32Len; uIdx++)
    {
        ...
        // Send the character to the UART output.
        //
        MAP_UARTCharPut(g_ui32Base, *(pu8_data + uIdx));
    }
...
//*****************************************************************************
//
//! Looks ahead in the receive buffer for a AT respone pattern.
//!
//! This function, available only when the module is built to operate in
//! buffered mode using \b UART_BUFFERED, may be used to look ahead in the
//! receive buffer for a particular AT respone pattern and report its position 
//! if found.
//!
//! <CR><LF><respone><CR><LF>
//!
//! \return Returns -1 to indicate that the AT respone is not completed
//! in the receive buffer.  Returns a non-negative number if the pattern was
//! found in which case the value represents the position of the end 
//! of \e pattern relative to the receive buffer read pointer.
//
//*****************************************************************************
#if defined(UART_BUFFERED) || defined(DOXYGEN)
int
UARTPeekATResponePattern(uint8_t* pu8_data, uint16_t u16_size_max)
{
    int iCount = 0;
    int iAvail = 0;
    uint32_t ui32ReadIndex = 0;
		bool b_ret = false;
    ...
    // <CR><LF><respone><CR><LF>
		// iAvail must be >= AT_PATTERN_SIZE_MIN
		if(iAvail >= AT_PATTERN_SIZE_MIN)
		{
			// Discard <CR><LF> begin of AT respone
			if(g_pcUARTRxBuffer[ui32ReadIndex] == CR)
			{
				if(g_pcUARTRxBuffer[(ui32ReadIndex + 1) % UART_RX_BUFFER_SIZE] == LF)
				{
					ADVANCE_RX_BUFFER_INDEX(g_ui32UARTRxReadIndex);
					ADVANCE_RX_BUFFER_INDEX(g_ui32UARTRxReadIndex);
					if(g_pcUARTRxBuffer[(ui32ReadIndex + 2) % UART_RX_BUFFER_SIZE] == '>')
					{
						*pu8_data = '>';
						return 1;
					}
					// Re-calculte number of characters in the rx buffer
					iAvail = (int)RX_BUFFER_USED;
					ui32ReadIndex = g_ui32UARTRxReadIndex;
				}
			}				
			// Check all the unread characters looking for the <CR><LF>.
			for(iCount = 0; iCount < iAvail; iCount++)
			{
				if(g_pcUARTRxBuffer[ui32ReadIndex] == CR)
				{
					// We found <CR>, check <LF> in next index
					if(g_pcUARTRxBuffer[(ui32ReadIndex + 1) % UART_RX_BUFFER_SIZE] == LF)
					{
						// We found the <CR><LF>
						b_ret = true;
						if(iCount < u16_size_max)
						{
							*(pu8_data + iCount) = g_pcUARTRxBuffer[ui32ReadIndex];
						}						
						if((iCount + 1) < u16_size_max)
						{
							*(pu8_data + iCount + 1) = 
							g_pcUARTRxBuffer[(ui32ReadIndex + 1) % UART_RX_BUFFER_SIZE];
						}						
						break;
					}
				}
		...
		if((b_ret == true) && (iCount > 0))
		{
			// Increase iCount 2 byte for <CR><LF>
			iCount += 2;
			return iCount;
		}
		else
...
//*****************************************************************************
//
//! Looks ahead in the receive buffer for a number of data.
//!
//! This function, available only when the module is built to operate in
//! buffered mode using \b UART_BUFFERED, may be used to look ahead in the
//! receive buffer for a particular number of data and report its position 
//! if found.
//!
//! <data><CR><LF>
//!
//! \return Returns false to indicate that the data is not completed
//! in the receive buffer.  Returns true if the data is completed
//! of \e pattern relative to the receive buffer read pointer.
//
//*****************************************************************************
#if defined(UART_BUFFERED) || defined(DOXYGEN)
bool
UARTPeekData(uint8_t* pu8_data, uint16_t u16_data_len)
{
    int iCount = 0;
    int iAvail = 0;
    uint32_t ui32ReadIndex = 0;
    ...
    iAvail = (int)RX_BUFFER_USED;
		...
		// iAvail must be >= u16_data_len
		if(iAvail >= u16_data_len)
		{			
			// Check all the unread characters looking for the <CR><LF>.
			for(iCount = 0; iCount < iAvail; iCount++)
			{
				if(iCount < u16_data_len)
				{
					*(pu8_data + iCount) = g_pcUARTRxBuffer[g_ui32UARTRxReadIndex];
				}
				// This one didn't match so move on to the next character.
				ADVANCE_RX_BUFFER_INDEX(g_ui32UARTRxReadIndex);
			}
...
//*****************************************************************************
//
//! Flushes the receive buffer at delta_index.
//!
//! This function, available only when the module is built to operate in
//! buffered mode using \b UART_BUFFERED, may be used to discard data
//! received from g_ui32UARTRxReadIndex to g_ui32UARTRxReadIndex + delta_index
//!
//! \return None.
//
//*****************************************************************************
#if defined(UART_BUFFERED) || defined(DOXYGEN)
void
UARTFlushRxAtDeltaIndex(uint32_t u32_delta_index)
{
    uint32_t ui32Int;
...
//    ui32Int = MAP_IntMasterDisable();
    ...
    // Flush the receive buffer.
    g_ui32UARTRxReadIndex = (g_ui32UARTRxReadIndex + u32_delta_index) %
														 UART_RX_BUFFER_SIZE;
