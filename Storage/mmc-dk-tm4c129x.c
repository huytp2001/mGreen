...
static xSemaphoreHandle x_ff_mutex;
static uint8_t u8_ff_select = FF_SELECT_NONE;
...
static uint32_t g_ui32SysClock = SYSTEM_CLOCK_GET();
...
//	if (u8_ff_select == FF_SELECT_USB)
//		return usb_disk_initialize(drv);
...
{
//	if (u8_ff_select == FF_SELECT_USB)
//		return usb_disk_read(drv, buff, sector, count);
...
{
//	if (u8_ff_select == FF_SELECT_USB)
//		return usb_disk_write(drv, buff, sector, count);
...
//	if (u8_ff_select == FF_SELECT_USB)
//		return usb_disk_ioctl(drv, ctrl, buff);
...
StaticSemaphore_t xMutex_ff_Buffer;
void v_ff_init(void)
{
	/* Create sd card mutex. */
	x_ff_mutex = xSemaphoreCreateMutexStatic(&xMutex_ff_Buffer);
}

int i16_ff_take_semaphore(int block_time, bool SDnUSB)
{
	if (xSemaphoreTake(x_ff_mutex, block_time) == pdTRUE)
	{
		if (SDnUSB)
			u8_ff_select = FF_SELECT_SD_CARD;
		else
			u8_ff_select = FF_SELECT_USB;
		return 0;
	}
	return -1;
}

int i16_ff_give_semaphore(void)
{
	u8_ff_select = FF_SELECT_NONE;
	if (xSemaphoreGive(x_ff_mutex) == pdTRUE)
	{
		return 0;
	}
	return -1;
